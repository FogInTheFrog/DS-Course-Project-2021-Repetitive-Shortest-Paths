...
from graph_representation import draw_points_and_edges, draw_points_depending_on_value, draw_mountain_ranges
import math
import numpy as np
...
def generate_mountain_ranges(numberOfMountains: int, map_width: int, map_height: int) -> list[((int, int), (int, int))]:
    ...
    mountainLength = min(min(map_width, map_height) // 2, 3500)            # units
...
def scale_height(dist_from_mountain):
    ...
    MOUNTAIN_RANGE_IMPACT = 35000
    MOUNTAIN_HEIGHT = 2500

    factor = min(max((MOUNTAIN_RANGE_IMPACT - dist_from_mountain) / MOUNTAIN_RANGE_IMPACT, 0), 1)  # or 3500

    percentile = factor ** 20  # This formula describes the best height profile of Poland
    return int(percentile * MOUNTAIN_HEIGHT)
...
def calculate_height(mountainRange, point):
    ...
    for mountain in mountainRange:
        ...
        if is_point_inside_segment(dist_src_dest, dist_src_point, dist_dest_point, dist_from_line):
            height += scale_height(int(dist_from_line))
        else:
            height += scale_height(int(min(dist_src_point, dist_dest_point)))
...
def sample_points_for_heatmap(mountainRanges, mapHeight, mapWidth):
    ...
    draw_mountain_ranges(points, edges)

    for i in range(0, mapWidth, 200):
        for j in range(0, mapHeight, 200):
            height = calculate_height(mountainRanges, (i, j))
