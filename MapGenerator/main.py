...
from triangulation import delaunay_triangulation, find_and_union, convex_hull, \
    ...
    find_center_of_polygon, distance_between_two_points_including_height
from graph_representation import draw_points_and_edges
from altitude_generator import generate_mountain_ranges, calculate_height, sample_points_for_heatmap

lowestUniqueNodeId = 1
lowestUniqueEdgeId = 1

VERTICAL_NUMBER_OF_REGIONS = 520
HORIZONTAL_NUMBER_OF_REGIONS = 520
REGION_HEIGHT = 320
REGION_WIDTH = 320
...
STARTING_POINT = (0, 0)
DEBUG = False
IS_GENERATING_MOUNTAINS = True
IS_GENERATING_POINTS = False
IS_SHOW_GENERATED_MOUNTAINS = True


# Function reads 5 arguments from standard input
def read_input() -> (str, str, int, int, int):
    print("Insert name of file to store points: ")
    nameOfFileToStorePointsRaw = input()

    print("Insert name of file to store edges: ")
    nameOfFileToStoreRoadsRaw = input()

    print("Insert number of mountain ranges to store edges: ")
    numberOfMountainRangesRaw = input()

    print("Insert height (number of regions along vertical axis): ")
    numberOfRegionsVerticallyRaw = input()

    print("Insert width (number of regions along horizontal axis): ")
    numberOfRegionsHorizontallyRaw = input()

    return nameOfFileToStorePointsRaw, nameOfFileToStoreRoadsRaw, int(numberOfMountainRangesRaw), \
           int(numberOfRegionsVerticallyRaw), int(numberOfRegionsHorizontallyRaw)
...
if __name__ == '__main__':
    (nameOfFileToStorePoints, nameOfFileToStoreRoads, numberOfMountainRanges, numberOfRegionsVertically,
     numberOfRegionsHorizontally) = read_input()

    # Update global values
    HORIZONTAL_NUMBER_OF_REGIONS = numberOfRegionsHorizontally
    VERTICAL_NUMBER_OF_REGIONS = numberOfRegionsVertically

    numberOfRegions = HORIZONTAL_NUMBER_OF_REGIONS * VERTICAL_NUMBER_OF_REGIONS
    sumOfAllPoints = 0

    # Initialize structures to keep
    ...
    mountains = generate_mountain_ranges(numberOfMountainRanges, MAP_WIDTH, MAP_HEIGHT) if IS_GENERATING_MOUNTAINS else []

    if IS_SHOW_GENERATED_MOUNTAINS:
        sample_points_for_heatmap(mountains, MAP_WIDTH, MAP_HEIGHT)

    if not IS_GENERATING_POINTS:
        exit(0)
    ...
    # Print total number of points generated
